...
/*   Updated: 2019/07/22 20:56:49 by solefir          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				read_and_valid(t_list *input)
{
	...
	buf = NULL;
	head = input;
	while (get_next_line(fd, &buf) != 0 && (size_buf = ft_strlen(buf)) > 0)
	{
		if (size_buf < 0 || buf[0] == '\n' || buf[0] == '\0')
			return (0);
		if (is_comment(&buf))
		...
		if (is_comand(buf) && (g_start > 1 && g_end > 1))
			...
			return (4);
		input->next = ft_lstnew((void*)buf, size_buf);
		input = input->next;
		//printf("buf %s\n", buf);
		//printf("con %s\n", (char*)input->content);
		ft_strdel(&buf);
